...
export function totalSetoranAnggota(kas, anggotaId) {
  const perBulan = kas?.setoran?.[anggotaId] || {};
  return Object.values(perBulan).reduce(
    (s, minggu) => s + Object.values(minggu).reduce((ss, v) => ss + (Number(v) || 0), 0),
    0
  );
}
